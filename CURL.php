...
class CURL{
    ...
    private $CLOUDFLARE_ENTERPRISE_DOMAIN = "namu.wiki";
    ...
    public function SEND(){
        ...
        if($this->OPTIMIZE_CLOUDFLARE || !empty($ENTERPRISE_REGION_ADDR)) curl_setopt($CURL, CURLOPT_RESOLVE, [$HOST.":443:".$ENTERPRISE_REGION_ADDR]);
        ...
        try{
            ...
            curl_close($CURL);
            if($HTTP_CODE == 0) return self::RETURN("ERR", curl_error($CURL), curl_errno($CURL));
            $FORMATED_HEADERS = array_reduce(explode("\n", substr($EXEC, 0, $HEADER_SIZE)), function($carry, $header) {
                list($key, $value) = array_pad(explode(': ', $header, 2), 2, null);
                if($key && $value) $carry[trim($key)] = trim($value);
                return $carry;
            }, []);
            return self::RETURN("OK", "FETCHED", $HTTP_CODE, array(
                "RESPONSE" => array("DATA" => substr($EXEC, $HEADER_SIZE), "HEADER" => $FORMATED_HEADERS),
